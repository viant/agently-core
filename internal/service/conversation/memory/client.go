...
func (c *Client) attachToolMessages(conv *agconv.ConversationView) {
	...
	for _, msg := range msgByID {
		if msg.ParentMessageId == nil || strings.TrimSpace(*msg.ParentMessageId) == "" {
			continue
		}
		parent := msgByID[strings.TrimSpace(*msg.ParentMessageId)]
		if parent == nil {
			continue
		}
...
// GetMessageByParentAndElicitation returns a child elicitation message whose parent is the proxy message.
func (c *Client) GetMessageByParentAndElicitation(_ context.Context, parentMessageID, elicitationID string) (*convcli.Message, error) {
	parentMessageID = strings.TrimSpace(parentMessageID)
	elicitationID = strings.TrimSpace(elicitationID)
	if parentMessageID == "" || elicitationID == "" {
		return nil, nil
	}
	c.mu.RLock()
	defer c.mu.RUnlock()
	for _, m := range c.messages {
		if m == nil || m.ParentMessageId == nil || m.ElicitationId == nil {
			continue
		}
...
func (c *Client) PatchModelCall(_ context.Context, in *convcli.MutableModelCall) error {
	...
	if _, ok := c.messages[in.MessageID]; !ok {
...
func copyMessage(m *agconv.MessageView) *agconv.MessageView {
	...
	if m.ToolMessage != nil {
		...
		for _, tm := range m.ToolMessage {
			...
			cp.ToolMessage = append(cp.ToolMessage, &tmc)
		}
	}
	return &cp
}
