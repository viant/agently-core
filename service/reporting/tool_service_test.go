...
func TestServiceMethodsExposeReportingSurface(t *testing.T) {
	...
	require.Equal(t, "compile_and_export_forge_ui", signatures[3].Name)
	require.Equal(t, "record_audit_event", signatures[4].Name)
	require.Equal(t, "share_artifact", signatures[5].Name)
	require.Equal(t, "transition_artifact", signatures[6].Name)
	require.Equal(t, "export_report", signatures[7].Name)
	require.Equal(t, reflect.TypeOf(&ExportJobStatus{}), signatures[7].Output)
	require.Equal(t, "submit_export", signatures[8].Name)
	require.Equal(t, reflect.TypeOf(&ExportJobStatus{}), signatures[8].Output)
	require.Equal(t, "get_export_status", signatures[9].Name)
	require.Contains(t, signatures[9].Description, "compact")
	require.Equal(t, reflect.TypeOf(&ExportJobStatus{}), signatures[9].Output)
	require.Equal(t, "list_export_jobs", signatures[10].Name)
	require.Equal(t, "list_export_artifacts", signatures[11].Name)
	require.Equal(t, "get_artifact", signatures[12].Name)
	require.Equal(t, "get_shared_artifact", signatures[13].Name)
	require.Equal(t, "list_shared_artifacts", signatures[14].Name)
	require.Equal(t, "save_report", signatures[15].Name)
	require.Equal(t, "get_report", signatures[16].Name)
	require.Equal(t, "list_reports", signatures[17].Name)
	require.Equal(t, "update_report", signatures[18].Name)
	require.Equal(t, "duplicate_report", signatures[19].Name)
	require.Equal(t, "delete_report", signatures[20].Name)
	require.Equal(t, "record_report_run", signatures[21].Name)
	...
	require.True(t, signatures[26].Internal)
}
