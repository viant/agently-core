...
func (s *Service) toolMessageResponseBody(ctx context.Context, tm *agconv.ToolMessageView) string {
	if tm == nil || tm.ToolCall == nil || tm.ToolCall.ResponsePayload == nil {
		if logx.Enabled() && tm != nil && tm.ToolCall != nil {
			...
			logx.Warnf("conversation", "agent.toolMessageResponseBody no payload for tool_call op_id=%q tool=%q", opID, toolName)
		}
		return ""
	}
	...
	if payloadID != "" && s != nil && s.conversation != nil {
		...
		if err != nil && logx.Enabled() {
			tn := ""
			if tm.ToolName != nil {
				tn = *tm.ToolName
			}
			logx.Warnf("conversation", "agent.toolMessageResponseBody GetPayload failed payload_id=%q tool=%q op_id=%q err=%v", payloadID, strings.TrimSpace(tn), strings.TrimSpace(tm.ToolCall.OpId), err)
		}
	...
	if logx.Enabled() {
		...
		logx.Warnf("conversation", "agent.toolMessageResponseBody empty body for tool_call op_id=%q tool=%q payload_id=%q", strings.TrimSpace(tm.ToolCall.OpId), strings.TrimSpace(tn), payloadID)
	}
	return ""
}
