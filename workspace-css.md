...
| --- | --- | --- | --- |
| `typography.family` | `--agently-theme-font-family` | `--forge-font-family` | Enum `system` or semantic `product-primary`; web hosts register and load the selected primary product-family assets once, while platforms without that asset use their system fallback |
...
Version 1 accepts literal `#RRGGBB` or `#RRGGBBAA` colors, bounded nonnegative numeric dimensions (font size and minimum height must be positive), and the documented typography enum. Define numeric bounds in the shared schema. Reject unknown tokens and wrong types with diagnostics. No CSS `var()`, `calc()`, font download URLs, or token-reference expressions in portable tokens. Product-family assets are application dependencies, not workspace CSS resources: a web host registers the `product-primary` alias, loads its assets once, and both generated application and Forge variables consume that alias. The role name does not constrain a future replacement to a sans-serif family. CSS files remain the web extension point for selectors and custom variables, with the existing parser validation. Native must never parse or execute those files and falls back to its system family when the selected product-family asset is unavailable.
