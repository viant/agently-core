...
func TestService_Load_StarterTasks(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, got)
	require.Len(t, got.StarterTaskCategories, 1)
	assert.Equal(t, "build", got.StarterTaskCategories[0].ID)
	assert.Equal(t, "Build", got.StarterTaskCategories[0].Title)
	assert.Equal(t, "Create or improve something.", got.StarterTaskCategories[0].Description)
	assert.Equal(t, "rocket", got.StarterTaskCategories[0].Icon)
	require.Len(t, got.StarterTasks, 2)

	assert.Equal(t, "analyze-repo", got.StarterTasks[0].ID)
	assert.Equal(t, "build", got.StarterTasks[0].CategoryID)
...
func TestService_Load_Intake_Orchestrator(t *testing.T) {
	...
	assert.Equal(t, "clarify", cfg.PlannerSecondFailurePolicy)
	assert.Equal(t, []string{"creative", "exploratory"}, cfg.PlannerTriggerPhrases)
